...
// Create client with better configuration for large uploads
...
export const upload3DModel = async (file, userId) => {
  try {
    ...
    // Upload with proper configuration for large files
    ...
    if (error) {
      ...
      throw new Error(error.message || 'Failed to upload 3D model');
    }
